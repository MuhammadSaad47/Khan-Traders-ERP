...
import { useSupplierStatement } from '../../hooks/useParties'
...
export function SupplierStatementDialog({ open, onOpenChange, supplierId, supplierName, fromDate, toDate }: Props) {
  const { data, isLoading, isError, error } = useSupplierStatement(supplierId || 0, fromDate, toDate)

  const handleDownloadPDF = () => {
    const element = document.getElementById('supplier-statement-content');
    if (!element) return;
    
    const opt: any = {
      margin:       0.5,
      filename:     `Supplier_Statement_${supplierName.replace(/\s+/g, '_')}.pdf`,
  ...
  const handlePrint = () => {
    const printContent = document.getElementById('supplier-statement-content');
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col p-0 overflow-hidden bg-background">
        <DialogHeader className="flex flex-row items-center justify-between p-6 pb-4 border-b pr-10">
          <DialogTitle className="text-xl">Supplier Statement</DialogTitle>
        ...
        <div className="overflow-y-auto p-6 flex-1 bg-muted/20">
          <div id="supplier-statement-content" className="bg-white p-8 rounded-xl shadow-sm border max-w-3xl mx-auto text-black">
            ...
            <div className="flex justify-between items-start mb-8 border-b pb-8">
              ...
              <div className="text-right">
                <h3 className="text-xl font-bold text-primary">SUPPLIER STATEMENT</h3>
                <div className="mt-2 space-y-1 text-sm text-gray-600">
                  <p><span className="font-semibold w-24 inline-block">Supplier:</span> {supplierName}</p>
